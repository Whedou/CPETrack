...
from lib.train.actors import CPETrackActor
# for import modules
import importlib
...
def run(settings):
    settings.description = 'Training script for CPETrack'
    ...
    # Create network
    if settings.script_name == "cpetrack":
        net = build_cpetrack(cfg)
    else:
    ...
    if settings.script_name == "cpetrack":
        ...
        actor = CPETrackActor(net=net, objective=objective, loss_weight=loss_weight, settings=settings, cfg=cfg)
    else:
        raise ValueError("illegal script name")
