...
from lib.train.dataset import VisEvent, LasHeR, DepthTrack, VTUAV
...
def names2datasets(name_list: list, settings, image_loader):
    ...
    for name in name_list:
        assert name in ["LASOT", "GOT10K_vottrain", "GOT10K_votval", "GOT10K_train_full", "GOT10K_official_val", "COCO17", "VID", "TRACKINGNET",
                        "DepthTrack_train", "DepthTrack_val", "LasHeR_all", "LasHeR_train", "LasHeR_val", "VisEvent", "VTUAV_train"]
        if name == "VTUAV_train":
            vtuav_root = os.environ.get("VTUAV_TRAIN_ROOT", settings.env.vtuav_train_dir)
            datasets.append(VTUAV(vtuav_root, dtype='rgbrgb'))
...
def get_optimizer_scheduler(net, cfg):
    ...
    addtion_param = [
        ...
        "input_pixel_fusion",
        "token_routers",
        "layer_injection_logits",
    ]
    param_dicts = [
        {"params": [p for n, p in net.named_parameters() if any(element in n for element in addtion_param) and p.requires_grad]},
        {
            "params": [p for n, p in net.named_parameters() if not any(element in n for element in addtion_param) and p.requires_grad],
